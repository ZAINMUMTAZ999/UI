...
import {AddJobTypes,jobSearchResponse} from "./types/AddJobTypes";
...
const AllUserFetching = async () => {
  try {
    ...
    console.log("Axios-userInfo", resp);
    

    return resp.data;
  } catch (error) {
    if (axios.isAxiosError(error)) {
      throw new Error("useInfo not found");
    }
    throw error;
  }
...
const updateMyProfileById = async (hotelFormData: FormData) => {
  const response = await fetch(`${Base_Url_API}/UpdateUser`, {
    method: "PUT",
    body: hotelFormData,
    credentials: "include",
  });
